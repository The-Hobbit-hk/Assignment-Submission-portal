...
export function PageHero({
  ...
  /** Optional one-line credibility note shown below the subtitle */
  trustLine?: string;
  className?: string;
}) {
  return (
    ...
    >
      {backgroundImage ? (
        <>
          <Image
            ...
            priority
            sizes="100vw"
            className="object-cover object-[center_28%] opacity-[0.58] sm:object-[center_32%]"
            aria-hidden
          />
          <div className="pointer-events-none absolute inset-0 bg-gradient-to-b from-white/30 via-white/44 to-white/58" />
        </>
      ) : (
        <div
      ...
      <div
        className={cn(
          ...
          backgroundImage &&
            large &&
            "depth-panel mx-4 max-w-3xl border border-white/80 bg-white/72 px-6 py-8 backdrop-blur-md sm:mx-auto sm:px-10",
          backgroundImage && large
            ? "py-10 pb-14 sm:py-12 sm:pb-16 lg:py-14 lg:pb-16"
            : "py-5 pb-6 sm:py-6 sm:pb-7"
        )}
      >
        <p className="site-hero-eyebrow mb-2 inline-flex items-center gap-2 rounded-full border border-accent/20 bg-white/80 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.2em] text-accent shadow-sm backdrop-blur-sm sm:text-xs">
        ...
        {trustLine && (
          <p className="mx-auto mt-3 max-w-xl text-xs font-medium text-zinc-500 sm:text-sm">
            {trustLine}
          </p>
        )}
