...
import { getCycleWindowState, isCycleOpen } from "@/lib/bluebook-labels";
...
export function serializeCycle(cycle: BluebookCycle) {
  const now = new Date();
  const windowState = isSubmissionWindowsBypassEnabled()
    ? ("open" as const)
    : getCycleWindowState(cycle.closesAt, cycle.opensAt, now);
  return {
    id: cycle.id,
    title: cycle.title,
    month: cycle.month,
